...
void newrace(int index, tCarElt* car, tSituation *s)
{
	...
	AbsWriteFd = open("/dev/ttyACM0", O_RDWR);
	if(AbsWriteFd)
		printf("PLAYER: Serial port opened for ABS Write: Fd = %d\n", AbsWriteFd);
	else 
		printf("Unable to open ABS Write Serial port\n");

	AbsReadFd = open("/dev/ttyUSB0", O_RDWR | O_NONBLOCK);
	if(AbsReadFd)
		printf("PLAYER: Serial port opened for ABS Read: Fd = %d\n", AbsReadFd);
	else 
		printf("Unable to open ABS Read Serial port\n");

	SteerWriteFd = open("/dev/ttyACM1", O_RDWR);
	if(SteerWriteFd)
		printf("PLAYER: Serial port opened for Steering Write: Fd = %d\n", SteerWriteFd);
	else 
		printf("Unable to open Steering Write Serial port\n");

	SteerReadFd = open("/dev/ttyUSB1", O_RDWR | O_NONBLOCK);
	if(SteerReadFd)
		printf("PLAYER: Serial port opened for Steering Read: Fd = %d\n", SteerReadFd);
	else 
		printf("Unable to open Steering Read Serial port\n");
...
static void common_drive(int index, tCarElt* car, tSituation *s)
{
	...
	static float float_sensitivity = 0.8;
	char steerbuf[4] = {0};
	char readSteerBuffer;
	...
	switch (cmd[CMD_LEFTSTEER].type) {
		case GFCTRL_TYPE_JOY_AXIS:
			...
			//printf("-----------Left steer ax0:%f, cmd.sen:%f, pub.speed:%f, spdSend:%f\n", ax0, cmd[CMD_LEFTSTEER].sens, car->pub.speed, cmd[CMD_LEFTSTEER].spdSens);
			...
			/* ----------------------------------------------Send pub.speed and Recive steering sensitivity from hercules------------------------------  */
			// Limit the frequency of this value to ECU

			if(count == 100)
			{
				sprintf(steerbuf, "%d", (int)car->pub.speed);
				write(SteerWriteFd, steerbuf, 4);

				read(SteerReadFd, &readSteerBuffer, 1);
				sensitivity = atoi(&readSteerBuffer);
				//printf("%d\n", sensitivity);
				count = 0;
			}
			else
			...
			if(sensitivity  == 2)
				...
				float_sensitivity = 0.8;

			cmd[CMD_LEFTSTEER].sens = float_sensitivity;

			/*
			if(car->pub.speed < 20.0)
				cmd[CMD_LEFTSTEER].sens = 1.0;
			else
				cmd[CMD_LEFTSTEER].sens = 0.8;
			*/
	...
	switch (cmd[CMD_RIGHTSTEER].type) {
		case GFCTRL_TYPE_JOY_AXIS:
			...
			//printf("-----------Right steer ax0:%f, cmd.sen:%f, pub.speed:%f, spdSend:%f\n", ax0, cmd[CMD_RIGHTSTEER].sens, car->pub.speed, cmd[CMD_RIGHTSTEER].spdSens);
			...
			/* Update the sensitivity recieved from Hercules in left steering to right steering  */





			cmd[CMD_RIGHTSTEER].sens = float_sensitivity;
		...
		if (fabs(car->_speed_x) > 10.0)
		{
			...
			*/
			...
			for (i = 0; i < 4; i++) {
				...
				sprintf(buf, "%d", (int)car->_wheelSpinVel(i));
				write(AbsWriteFd, buf, 4);
			}

			for(i = 0; i < 4; i++) {
				read(AbsReadFd, &readBuffer[i], 1);
			}
			slip_int = atoi(readBuffer);
			slip = (float)slip_int;
